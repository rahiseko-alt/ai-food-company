...
4. **nav を上から辿る**（機械強制の有効化＝手順0 → ヒアリング/リサーチ/提案/承認 → 設定 →
   **ゴール定義ゲート** → 原子ツリー作成）。
5. nav の通り、下記「案件の絶対起点」に従って**ゴール定義ゲート**（`docs/goal-gate.md`）を通してから、
   `docs/roadmap.html` を**原子ツリーへ描き直す**（`ROOT` は破棄）。
...
冒頭の「案件の絶対起点」どおり、**最後（7）の原子ツリー作成が着手の第一成果物**であり、
これを飛ばして実装に入ることは無い（1〜6 はツリーを描くための入力）。
...
5. **設定** — 承認された構成を、その案件の `AGENTS.md`（`presets/_TEMPLATE.md` を複製）に
   記載し、必要な雛形を作る。
6. **ゴール定義ゲートを通す（正＝`docs/goal-gate.md`）** — 原子ツリーを描く直前の関所。
   サブA(`goal-drafter`)に8問へ答えさせ → サブB(`basis-reviewer`)に却下条件を1個ずつ当てさせ →
   **B が出した争点ごとに、あなた（人間）が「直させる／通す（理由を記録）」を選ぶ**。
   突き合わせ3本（CHK1〜3）が1本でも切れていればゴール未確定＝7に進まず A に書き直させる。
   結果は `docs/roadmap.html` の `meta.goalgate` に残す。**A と B は必ず別のサブエージェントにする**
   （同一セッションが書いて採点する＝本人採点は禁止）。
7. **原子ツリーを作る（必須・第一成果物）** — 案件のゴールを頂点に、`docs/roadmap.html` の JSON を
   **原子レベルまで分解したツリー**として描き直す（コピー元の残骸ノードは破棄）。各葉に `criteria`/`verify`
   を置き、`meta.active`/`meta.next`/`meta.handoff` を初期化する。**実装作業の実行はこのツリー確定後**。
   ※ 純粋な調査・マニュアル作成など「スタック不問」の案件は 1〜5 を最小化してよいが、**6 と 7 は省けない**。
